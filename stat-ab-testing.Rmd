...
# 1. 데이터 ----
ab_dat <- read_delim("data/ab_df.txt", delim=' ') 
...
ab_dat %>% 
  mutate(month = month(play_date, label = TRUE)) %>% 
  group_by(month) %>% 
  summarise(좋아요율 = mean(like_click)) %>% 
    ggplot(aes(x=month, y=좋아요율, group=1)) + 
      geom_line() +
      geom_point(size=3) +
      scale_y_continuous(limits = c(0,1), labels = scales::percent) +
      labs(x="월", y="좋아요 비율(%)", title="월별 전환율 추세") +
